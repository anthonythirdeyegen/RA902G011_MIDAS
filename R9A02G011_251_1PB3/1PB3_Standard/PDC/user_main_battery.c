...
UCHAR user_func_chk_battery (void)
{
	...
	switch (gBatInfo.uSt.bSt) {
		case BATTERY_INIT:
			...
			gBatInfo.ucBatteryLevel = BATT_LEVEL_INVALID;
			gBatInfo.uReq.bDSleepExit = 0U;			
#if ROLE_SW_EN
		...
		case BATTERY_INIT_IDLE:
			if (gBatInfo.uIntr.bIntrSt == BATTERY_INTR_IDLE) {
				...
				if (gBatInfo.ucBatteryLevel == BAT_LEVEL_EMP){
				...
				switch (gBatInfo.uSt.bSubSt) {
					case BATTERY_EEM_SUB_S1:
						if (   (gBatInfo.uReq.bFixedRole != 0U)
							&& (gBatInfo.ucBatteryLevel  != BAT_LEVEL_EMP)) {
...
#if ROLE_SW_EN
						if (gBatInfo.uReq.bDSleepExit != 0U) {
...
#if ROLE_SW_EN
	if ( ((gBatInfo.uSt.bSt == BATTERY_EXIT_EMPTY_MODE) && (gBatInfo.uSt.bSubSt == BATTERY_EEM_SUB_S3))
	   || (gBatInfo.uSt.bSt == BATTERY_IDLE)) {
		ret = PD_CORE_STOP;
	}
#else
...
UCHAR user_func_battery_deepsleep(void)
{
#if ROLE_SW_EN
	PD_STATUS uStatus   = pdc_get_status();
	if (  (uStatus.bit.bPlug == 0U) && ((gBatInfo.uReq.bDSleepExit == 0U) || (gBatInfo.uSt.bSt == BATTERY_IDLE))) {
		return 1U;
	}
#endif
	return 0U;
...
{
	return gBatInfo.ucBatteryLevel;
}
		...
		case BATTERY_INTR_S8:
			...
			if (battTemp >= 0) { 
				...
				if (VAL_BATTVOLTH3 < tmpVolt) {
					gBatInfo.ucBatteryLevel = BAT_LEVEL_FULL;
				}
				else if (VAL_BATTVOLTH2 < tmpVolt) {
					gBatInfo.ucBatteryLevel = BAT_LEVEL_HIGH;
				}
				else if (VAL_BATTVOLTH1 < tmpVolt) {
					gBatInfo.ucBatteryLevel = BAT_LEVEL_MID;
				}
				else if (VAL_BATTVOLTH0 < tmpVolt) {
					gBatInfo.ucBatteryLevel = BAT_LEVEL_LOW;
				} 
				else {
					gBatInfo.ucBatteryLevel = BAT_LEVEL_EMP;
				}
