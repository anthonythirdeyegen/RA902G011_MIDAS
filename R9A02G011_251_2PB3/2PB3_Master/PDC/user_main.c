...
void user_init(void)
{
	...
	// slave int (INTP3)
	P3_bit.no2 = 0U; PM3_bit.no2 = 1U; PMK3 = 0U; // slave int :P32(INTR3)
	EGP0 |= 0x04;
	EGN0 |= 0x06;
...
void user_func_event (void)
{
	...
	if (gPdc.uPdEvent.bit.bPlugChg != 0U) {
		...
		if(gDCInfo.uReq.usData == 0U){
			...
			if (uStatus.bit.bPR == 0U) { // ATT.NonPD-SNK
				gMpPb.uStat.bit.bChgEn = 1U;
				gDCInfo.uReq.bit.bSnkChg = 1U;
				gMppmInfo.portInfo[MPPM_PORT_MASTER].portFlag.bit.bAttSnk = 1U;
				...
				if (((gMpPb.othPortInfo.bit.bPlugState == 0U)
					|| ((gMpPb.othPortInfo.bit.bPlugState == 1U) && (gMpPb.othPortInfo.bit.bPowerRole == 1U)))
					&& (gMppmInfo.portInfo[MPPM_PORT_MASTER].portFlag.bit.bTrickle != 0U)) {
					gMppmInfo.portEvent[MPPM_PORT_MASTER].bit.bChkTrickle = 1U;
				}
	...
	if (gPdc.uPdReq.bit.bVconnDis != 0U) {
		...
		if (sVbusTmp >= 4000) {
			if (gDCInfo.uReq.usData == 0U) {
				...
				if ((gMpPb.othPortInfo.bit.bPlugState != 0U)
					...
					if (gMpPb.othPortInfo.bit.bAttachOrder != 0U) {
					...
					if (gMpPb.othPortInfo.bit.bAttachOrder == 0U) { //Master-2nd, Slave-1st
						gMpPb.uStat.bit.bResetDcdcComp = 0U;
						gMpPb.uEvent.bit.bResetDcdcComp = 0U;
						gDCInfo.uReq.bit.bSnkChg = 1U;
						gMpPb.uStat.bit.bChgEn = 1U;

						if ((gMpPb.gulPSRC1 >= gMpPb.gulPBATC) || (gMppmInfo.portEvent[MPPM_PORT_SLAVE1].bit.bChkTrickle != 0)) {
							gMpPb.uStat.bit.bChgEn = 0U;
						}
					}
					else { //Master-1st, Slave-2nd
						if ((gMpPb.gulPSRC1 < gMpPb.gulPBATC) && (gMpPb.gulPSRC2 >= gMpPb.gulPBATC)) {
							gMpPb.uStat.bit.bChgEn = 0U;
		...
		gMpPb.uEvent.bit.bResetDcdcComp = 0U;
		if (gMpPb.othPortInfo.bit.bAttachOrder == 1U) {
			gMppmInfo.portEvent[MPPM_PORT_MASTER].bit.bChgDcdc = 1U;
		}
	}
		...
		if (gDCInfo.uReq.usData == 0U) {
			...
			if (gBatInfo.uSt2.bTrickle == 1U) {
				//Non-Trickle(0U) -> Trickle(1U)
				...
				if ((gMpPb.othPortInfo.bit.bPlugState == 1U) 
					...
					gMpPb.gulPSRC1 = isl95538b_get_psrc_other();//Slave 1st Att.Snk
					gMpPb.gulPBATC = isl95538b_get_pdatc();
